...
namespace Filtro_de_Ramos.Vista
{
    public partial class FormSede : Form
    {
        ...
        public FormSede()
        {
            ...
            swbn = (int)Properties.Settings.Default["Sede"];
            if (swbn == 1)
            {
        ...
        {
            Sede sede = new Sede { Nombre = TB_Nombre.Text };
            controladorSede.Agregar(sede);
            TB_Id.Text = sede.Id.ToString();
            DGV_Tabla.DataSource = controladorSede.ConsultarPorUno(sede.Id, null);
        ...
        {
            Sede sede = new Sede { Id = int.Parse(TB_Id.Text), Nombre = TB_Nombre.Text };
            controladorSede.Actualizar(sede);
        }
        private void B_Eliminar_Click(object sender, EventArgs e)
        {
            if (TB_Id.TextLength == 0)
                ...
                controladorSede.Eliminar(int.Parse(TB_Id.Text));
        }
        private void B_ConsultarPorUno_Click(object sender, EventArgs e)
        {
            if (TB_PalabraClave.TextLength == 0)
            {
                ...
                DGV_Tabla.DataSource = controladorSede.ConsultarPorTodos();
            }
            else
            if (!int.TryParse(TB_Id.Text, out id))
                DGV_Tabla.DataSource = controladorSede.ConsultarPorUno(0, TB_PalabraClave.Text);
            else
                DGV_Tabla.DataSource = controladorSede.ConsultarPorUno(int.Parse(TB_PalabraClave.Text), TB_PalabraClave.Text);
        }
        ...
        private void B_BN_Click(object sender, EventArgs e)
        {
            ...
            Properties.Settings.Default["Sede"] = swbn;
