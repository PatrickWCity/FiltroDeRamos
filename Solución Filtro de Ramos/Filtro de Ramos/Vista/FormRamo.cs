...
{
    public partial class FormRamo : Form
    {
        ...
        public FormRamo()
        {
            ...
            swbn = (int)Properties.Settings.Default["Ramo"];
            if (swbn == 1)
            {
        ...
        {
            Ramo ramo = new Ramo { Nombre = TB_Nombre.Text };
            controladorRamo.Agregar(ramo);
            TB_Id.Text = ramo.Id.ToString();
            DGV_Tabla.DataSource = controladorRamo.ConsultarPorUno(ramo.Id, null);
        ...
        {
            Ramo ramo = new Ramo { Id = int.Parse(TB_Id.Text), Nombre = TB_Nombre.Text };
            controladorRamo.Actualizar(ramo);
        }
        private void B_Eliminar_Click(object sender, EventArgs e)
        {
            if (TB_Id.TextLength == 0)
                ...
                controladorRamo.Eliminar(int.Parse(TB_Id.Text));
        }
        private void B_ConsultarPorUno_Click(object sender, EventArgs e)
        {
            if (TB_PalabraClave.TextLength == 0)
            {
                ...
                DGV_Tabla.DataSource = controladorRamo.ConsultarPorTodos();
            }
            else
            if (!int.TryParse(TB_Id.Text, out id))
                DGV_Tabla.DataSource = controladorRamo.ConsultarPorUno(0, TB_PalabraClave.Text);
            else
                DGV_Tabla.DataSource = controladorRamo.ConsultarPorUno(int.Parse(TB_PalabraClave.Text), TB_PalabraClave.Text);
        }
        ...
        private void B_BN_Click(object sender, EventArgs e)
        {
            ...
            Properties.Settings.Default["Ramo"] = swbn;
